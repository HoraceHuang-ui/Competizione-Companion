...
// 数据管道：ACC 进行局域网服务器发现时，DLL 从这里读取需要伪造的服务器列表
#define NAMED_PIPE_NAME L"\\\\.\\pipe\\competizione-acc-connector"
// 状态管道：DLL 被 ACC 加载后主动握手一次，通知宿主应用“注入已生效”
#define STATUS_PIPE_NAME L"\\\\.\\pipe\\competizione-acc-connector-status"
...
// 注入生效握手：best-effort 连接状态管道并写入版本信息，宿主应用未运行则静默失败
void notify_status(void) {
    HANDLE hPipe = CreateFileW(STATUS_PIPE_NAME, FILE_GENERIC_READ | FILE_GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
    ...
    const char* msg = "competizione-acc-connector " VERSION_STR;
    DWORD written = 0;
...
void handle_discovery(SOCKET s, DWORD id) {
    ...
    DWORD totalRead = 0;
    while (totalRead < sizeof(struct shared_memory)) {
        DWORD bytesRead = 0;
        if (!ReadFile(hPipe, (BYTE*)&shm + totalRead, (DWORD)(sizeof(struct shared_memory) - totalRead), &bytesRead, NULL) || bytesRead == 0) {
            break;
        }
        totalRead += bytesRead;
    }
