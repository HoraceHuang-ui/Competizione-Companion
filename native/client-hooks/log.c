...
void initLog() {
    ...
    if (SUCCEEDED(res)) {
        wchar_t path[512];
        swprintf_s(path, 512, L"%s\\Competizione Companion", documents_path);
        CreateDirectory(path, NULL);
        swprintf_s(path, 512, L"%s\\Competizione Companion\\logs", documents_path);
        CreateDirectory(path, NULL);
		swprintf_s(path, 512, L"%s\\Competizione Companion\\logs\\acc-hook.log", documents_path);
